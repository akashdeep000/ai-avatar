...
export const useInternalPlayback = ({ playbackQueue, aiState, dispatch, setExpression, startMotion }: UseInternalPlaybackProps) => {
    ...
    const playAudio = useCallback(async (audioBase64: string) => {
        ...
        const live2dManager = (window as { getLive2DManager?: () => { getModel: (index: number) => Live2DModel } }).getLive2DManager?.();
        if (live2dManager) {
